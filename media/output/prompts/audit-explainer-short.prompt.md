![audit-explainer-short.prompt](images/audit-explainer-short.prompt.png)

# Prompt: Fail-Closed Audit Explainer (video)
## Style & Brand
- Palette: {'primary': '#6C63FF', 'secondary': '#1976D2', 'accent': '#FFC107'}
- Typography: {'heading': 'Sans/Readable', 'body': 'Sans/Readable'}
## Accessibility
- Contrast target: WCAG AA
- Alt text required: True
- Captions required: True
## Context
- Project: Core Framework
- Files: ['README.md', 'COMPLIANCE.md', 'AUDIT-ARCHITECTURE.md', 'FRICTION-PROTOCOL.md']
- Image sources: ['/Users/bamdad/www/GenAI-RD/images', '/Users/bamdad/www/GenAI-RD/Alliance-to-Empire/images', '/Users/bamdad/www/GenAI-RD/AI-Guardrails-and-Bias/images', '/Users/bamdad/www/GenAI-RD/Economy/images', '/Users/bamdad/www/GenAI-RD/One-Shot/images', '/Users/bamdad/www/GenAI-RD/US-Gov/images', '/Users/bamdad/www/GenAI-RD/Ugly-Truth/images']
## Requirements
- Format: mp4 | Size/Duration: 60s
- Intended audience: Executives | Targets: ['execs']
## Draft Instruction Example
60s motion graphic explaining audit pipeline and fail‑closed posture; include captions.

## Three‑Model Review
- Model A: Generate candidates
- Model B: Review or provide options
- Model C: Decide and finalize (record rationale)